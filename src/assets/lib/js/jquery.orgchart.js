...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.positionId] ? ' data-id="' + nodeData[opts.positionId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      ...
      .append('<div class="title">' + nodeData[opts.positionName] + '</div>')
        ...
        <div class="info-comtainer">
          <div class="person-info-comtainer">
            <div class="person-info-name">
              ${nodeData[opts.employeeName]}
            </div>
            <div class="person-info-org">
              ${nodeData[opts.unitName]}
            </div>
          </div>
  ...
  function currentPositionInfo(data) {
      ...
      positionInfo.employeeArr = employeeArr;
      positionInfo.employeeName = employeeArr[0].fullName; //当前职位第一个员工的姓名
      positionInfo.photoId = photoUrl(employeeArr[0].photoId); //当前职位第一个员工的相片
      positionInfo.relationship = '111';
      positionInfo.unitName = data.unitName;
      return positionInfo;
  }
  ...
  function createChildrenData(data,isDirect) {
      ...
      children.forEach(function(item) {
          let addProperty = currentPositionInfo(item);
          if(!isDirect) {
