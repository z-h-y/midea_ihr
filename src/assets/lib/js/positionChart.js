...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    switch (options) {
      ...
      default: // initiation time
        var opts = $.extend(defaultOptions, options);  //合并orgchart传入的参数表和默认参数表
    }

    // build the org-chart

    var $chartContainer = this; //存放图普的盒子对象
    var data = opts.data;
    var $chart = $('<div>', {
      'data': { 'options': opts },
    ...
    if ($.type(data) === 'object') {
      ...
      $.ajax({
        ...
        headers: {
          'ewpAdminAuthorization' : localStorage.getItem('ewptoken')
        }
      })
      .done(function(data, textStatus, jqXHR) {
        let employeeData = transformData(data);
        buildHierarchy($chart, opts.ajaxURL ? employeeData : attachRel(employeeData, '00'), 0, opts);
      })
        ...
        'click': function() {
          ...
          html2canvas(sourceChart, {
            ...
            'onrendered': function(canvas) {
              ...
              let fileName = 'structureChart'+ new Date().getTime();
  ...
  function getNodeState($node, relation) {
    ...
    if ($target.length) {
      let isVisible = $target.is(':visible') && ($target.css('visibility') == 'visible');
      if (isVisible) {
  ...
  function hideChildren($node) {
    ...
    $visibleNodes.addClass('slide-up');
     $temp.addClass('hidden');
     return false;
  ...
  function showChildren($node) {
    ...
    $descendants.removeClass('slide-up');
    $descendants.closest('table').closest('tr').prevAll('.lines').css('visibility', 'visible');
    return false;
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.positionId] ? ' data-id="' + nodeData[opts.positionId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      .addClass('node ' + (nodeData.className || '') + (nodeData.isDirect || '') +  (level >= opts.depth ? ' slide-up' : ''))
      .append(`<div class="title"><p>${nodeData[opts.positionName]}</p></div>`)
      .append(
        `<div class="content">
          <div class="info-comtainer">
            <div class="person-info-comtainer">
              <div class="person-info-name" title="${nodeData['employeeLongName']}">
                <p>${nodeData[opts.employeeName]}</p>
              </div>
              <div class="person-info-org" title="${nodeData['unitLongName']}">
                <p>${nodeData[opts.unitName]}</p>
              </div>
            </div>
            <div class="person-info-pic">
              <img src="${nodeData[opts.photoId]}" />
            </div>
          </div>
        </div>`);
      if(nodeData['subNum'] > 0) {
        $nodeDiv.append(`<div class="subNum">${nodeData['subNum']}</div>`);
      }
    ...
    $nodeDiv.on('click', '.topEdge', function(event) {
      ...
      if (parentState.exist) {
        ...
        var nodeId = $that.parent().data('id');
    ...
    $nodeDiv.on('click', '.bottomEdge,.subNum', function(event) {
      event.stopPropagation();
      var $that = $(this);
      var $node = $that.parent();  //获取当前node节点的盒子对象
      ...
      if (childrenState.exist) {
        ...
        var nodeId = $that.parent().data('id');  //获取当前职位的ID

        if (startLoading($that, $node, opts)) {
          let getShortUrl = $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children(nodeId) : '';
          let url =   Vue.config.APIURL + getShortUrl;
          $.ajax({
              url,
              type: 'POST',
              'dataType': 'json',
          ...
          .done(function(data, textStatus, jqXHR) {
              let legalData = transformData(data); //处理后台请求的数据
            if ($node.closest('.orgchart').data('inAjax')) {
              if (legalData.children.length) {
                addChildren($node, legalData, $.extend({}, opts, { depth: 0 }));
              }
            }
          })
          .fail(function(jqXHR, textStatus, errorThrown) {
  ...
  //transform data from server as model
  function transformData(data) {
    let positionAll = {}; //生成当前员工信息
    let children = []; //当前节点的子节点
    positionAll = currentPositionInfo(data);
    let directNode = data.posDirectSubordinateDto;  //当前直线子节点
    let dottedNode = data.posDottedSubordinateDto;  //当前虚线子节点
    let directArr = createChildrenData(directNode,true); //创建直线子节点数据
    let dottedArr = createChildrenData(dottedNode,false); //创建虚线子节点数据
    children = directArr.concat(dottedArr);
    positionAll.children = children;
    return positionAll;
  }

  function currentPositionInfo(data,isChild) {
      let positionInfo = {}; //生成当前员工信息
      let employeeArr = data.empDtoList; //当前职位所有的员工
      if(!employeeArr.length) {
        ...
        employeeArr.unshift(empty);
      }
      positionInfo.positionId = data.positionId; //当前职位的ID
      positionInfo.positionName = data.positionName; //当前职位的名称
      positionInfo.employeeArr = selectEmployee(employeeArr);
      positionInfo.employeeName = textEllipsis(employeeArr[0].fullName, 260, {fontSize: '14px'}); //当前职位第一个员工的姓名
      positionInfo.employeeLongName = employeeArr[0].fullName;
      positionInfo.photoId = photoUrl(employeeArr[0].photoId); //当前职位第一个员工的相片
      positionInfo.relationship = '000';
      positionInfo.subNum = data.subordinateCount || 0;
      positionInfo.unitName = textEllipsis(data.unitName, 260, {fontSize: '14px'});
      positionInfo.unitLongName = data.unitName;
      return positionInfo;
  }

  function textEllipsis(string, widthLimit, textSize) {
      var Suffix = '....';
      var doc = document;
      var body = doc.getElementsByTagName("body")[0];
      var containerNode = doc.createElement("pre");
        var width = 0;
        for(var i in textSize) {
          if (textSize.hasOwnProperty(i)) {
              containerNode.style[i] = textSize[i];
          }
        }
  ...
  function selectEmployee(emp) {
    let empArr = [];
    let empNum = emp.length;
    if(emp[0].hasOwnProperty('delete')) {
        empNum--;
    }
    emp.forEach(function(list) {
        let empInfo = {}; 
        empInfo.photoId = photoUrl(list.photoId);
        empInfo.employeeName = list.fullName;
        empArr.push(empInfo);
        empArr.empNum = empNum;
    })
    return empArr;

  }

  function photoUrl(photoId) {  //获取图片的URL显示图片，当加载失败就从本地获取一张默认的图片
      if (photoId){
          return Vue.config.APIURL + `/system/attachment/downloadImg/${photoId}`;
      }else {
          return `/assets/images/public/default-header.png`;
      }
  }

  //create the property of children
  function createChildrenData(data,isDirect) {
      let children = data || [];
      let returnData = [];
      children.forEach(function(item) {
          let addProperty = currentPositionInfo(item,true);
          if(!isDirect) {
            addProperty.isDirect = "dotted";
          }

          returnData.push(addProperty);
      })
