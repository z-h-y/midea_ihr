...
(function(factory) {
  ...
  $.fn.orgHistoryChart = function(options) {
    var defaultOptions = {
      'nodeTitle': 'name',
      'nodeId': 'id',
      ...
      'newItem': 'origin',
      'exportButton': false,
      'exportFilename': 'orgHistoryChart',
    ...
    if ($.type(data) === 'object') {
      if (data instanceof $) { // ul datasource
        ...
        let queryDate = opts.querydate;
      $.ajax({
        'url': Vue.config.APIURL + data,
        'dataType': 'json',
        data: {
           queryDate: queryDate,
           parentUnitId: opts.searchId,
           unitId:  opts.searchId
        },
        'beforeSend': function () {
 ...
 function hideChildren($node) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    var dtd = $.Deferred();
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.unitId] ? ' data-id="' + nodeData[opts.unitId] + '"' : '') + '>')
      .addClass('node ' + (nodeData.className || '') +  (level >= opts.depth ? ' slide-up' : ''))
      .append(`<div class="title"><p>${nodeData[opts.orgShortName] ? nodeData[opts.orgShortName] : 'Organization'}</p></div>`)
      .append(
      `<div class="content">
        <div class="info-comtainer">
          <div class="person-info-comtainer">
            <div class="person-info-name" title="${nodeData['employeeLongName'] ? nodeData['employeeLongName'] : ''}">
              <p>${nodeData[opts.employeeName] ? nodeData[opts.employeeName] : ''}</p>
            </div>
            <div class="person-info-org" title="${nodeData['jobLongName'] ? nodeData['jobLongName'] : ''}">
             <p>${nodeData[opts.jobName] ? nodeData[opts.jobName] : ''}</p>

            </div>
          </div>
          <div class="person-info-pic">
            <img src="${nodeData[opts.imgUrl]}" />
          </div>
        </div>

      </div>`);
      if(nodeData['childCount'] > 0) {
        $nodeDiv.append(`<div class="child-num">${nodeData['childCount']}</div>`);
      }
    ...
    $nodeDiv.on('click', '.bottomEdge,.child-num', function(event) {
      event.stopPropagation();
      var $that = $(this);
      ...
      if (childrenState.exist) {
        ...
        if (startLoading($that, $node, opts)) {
          let getSearchUrl = $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children(nodeId) : '';
          let url =   Vue.config.APIURL + getSearchUrl;
          let queryDate = opts.querydate;
          $.ajax({
              url,
              'dataType': 'json',
  ...
  function transformData(resData) {
    let parentData = resData[0];
    let transfored = currentPositionInfo(parentData);
    let childrenArr = resData.slice(1);
    let children = createChildrenData(childrenArr);
    transfored.children = children; //获取当前职位子节点
    return transfored; //返回处理后的数据
  }
  function currentPositionInfo(data) {
      let newInfo = {};
      let fullName = data.fullUnitName;
      let shortName = '';
      newInfo.employeeName = textEllipsis(data.employeeName, 260, {fontSize: '14px'}); //职工名称
      newInfo.employeeLongName = data.employeeName; //职工名称
      newInfo.orgShortName = fullName.split('_').pop();  //获取组织的短称
      // newInfo.orgShortName = longText(shortName);
      newInfo.imgUrl = photoUrl(data.photoId);  //获取图片url
      newInfo.jobName = textEllipsis(data.oldjobName, 260, {fontSize: '14px'}); //职工职位名称
       newInfo.jobLongName = data.oldjobName; //职工职位名称
      newInfo.unitId = data.unitId; //所在组织ID
      newInfo.relationship = '000';
      newInfo.childCount = data.childCount; //子节点数量
      return newInfo;
  }
  ...
  function createChildrenData(data = []) {
      let children = [];
      data.forEach(function(list) {
          let child = currentPositionInfo(list);
          children.push(child);
      })
      return children;
  }
