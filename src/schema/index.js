...
import { formatDate, merge, getPath, getNestedPath } from '../util/assist';
// import { default as defaultMessages } from './messages';
// import { default as locales } from '../locales/index';

 // TODO Execute validation program
let doValidate = function(object, property, descriptor, rule) {
  ...
  if (typeof validateFn === 'function') {
    ...
    if (!message) {
      ...
      message = Vue.t('validator.' + type);
      // message = locales['en']['validator'][type] || defaultMessages[type];
...
// TODO init Object
let initObject = (object, schema, addMethod) => {
  ...
  if (addMethod) {
    ...
    Object.defineProperty(object, 'isFormValidate', {
      configurable: true,
      enumerable: false,
      value: function() {
        return this.$schema.isFormValidate(this);
      }
    });
...
class Schema {
  ...
  getPropertyMultiSelect(property) {
    return this.props[property].multiSelect || false;
  }
  ...
  isFormValidate(form, obj) {
    ...
    Object.assign(defaultObject, obj);
    form.isValidate = true;
    form.$broadcast('checkFormValidate',defaultObject);
    form.isFocus = false;
