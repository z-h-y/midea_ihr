  ...
  methods: {
    ...
    _addFileUploads(el) {
      ...
      if (el.files) {
        ...
        var file = [{size: -1, name: el.value.replace(/^.*?([^\/\\\r\n]+)$/, '$1')}];
        if(!this._checkFile(file)) {
          return;
        }
        this._addFileUpload({el:el}, {size: -1, name: el.value.replace(/^.*?([^\/\\\r\n]+)$/, '$1'), filter: "filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='scale',src='"+nfile+"')"});
      }
    },
    _checkFile(files) {
      var isValidate = true;
    ...
    _fileUploads() {
      ...
      if (this.files.length < 1) {
        this._addFileUpload({}, {size: -1, name: ''});
        hasFile = false;
      }
      var firstFile = this.files[0];
      if (this.$mode == 'html5' && this.multiple) {
        if (!this._isUpload()) {
          this._fileUploadHtml5(firstFile);
        } else {
          this.clear();
        }
        return;
      } else if (this.$mode == 'html4' && this.multiple) {
        if (!this._isUpload()) {
          this._fileUploadHtml4(firstFile);
        } else {
          this.clear();
        }
        return;
      ...
      for (var i = 0; i < this.files.length; i++) {
        ...
        if (this.size && this.size > 0 && file.size >= 0 && file.size > this.size && hasFile) {
          file.error = 'Size';
          file.errno = 'size';
          Message({type: 'error', message: this.$t('staff.message.fileUploadSize')});
          continue;
        ...
        if (this.extensions && (this.extensions.length || typeof this.extensions.length == 'undefined') && hasFile) {
          var extensions = this.extensions;
          if (typeof extensions == 'object' && extensions instanceof RegExp) {
          ...
          if (file.name !== '' && file.name.search(extensions) == -1) {
    ...
    _fileUploadHtml5(file) {
      ...
      if (this.multiple) {
        for (var i = 0; i < this.files.length; i++) {
          var file = this.files[i];
          if (this._files[file.id].file) {
            form.append(this.name + i, this._files[file.id].file);
          }
        }
      } else {
        if (this._files[file.id].file) {
      ...
      xhr.setRequestHeader('ewpAdminAuthorization', localStorage.getItem('ewptoken'));
    ...
    _fileUploadHtml4(file) {
      ...
      if (hiddenData.el && !this.multiple) {
        form.appendChild(hiddenData.el);
      }
      if (this.multiple) {
        for (var i = 0; i < this.files.length; i++) {
          var file = this.files[i];
          var el = this._files[file.id].el;
          if (el) {
            el.name = el.name + i;
            form.appendChild(this._files[file.id].el);
          }
        }
      }
      for (var key in this.request.data) {
        ...
        switch (e.type) {
          ...
          if (typeof data != "undefined") {
            if (data && data.substr(0, 1) == '{' && data.substr(data.length - 1, 1) == '}') {
              ...
              var reg = /<div>/ig;
              if (data.match(/<\/div>/ig)) {
                file.errno = 'Internal Server Error';
