            ...
            <ui-collapsible :open="true" v-for="menu in parentMenus" :header="menu.menuName">
                <table class="panel-content-table">
                    <tr v-for="m in getChildNodes(menu)">
                        ...
                        <td class="pl15">
                            <ui-checkbox :name="m.menuId" :model.sync="selIds" :value="m.menuId" checked>View</ui-checkbox>
                        </td>
                    </tr>
                </table>
            </ui-collapsible>
...
export default {
    data() {
        ...
        methods: {
            ...
            submit() {
                    let _self = this;
                    for (let m of _self.menus.roleMenuList) {
                        m.isHadPower = 'N'
                    }
                    for (var s of _self.selIds) {
                        for (let m of _self.menus.roleMenuList) {
                            if (s === m.menuId) {
                                m.isHadPower = 'Y'
                            }
                    ...
                    let data = {};
                    convert(this.menus, data, "", true);
                    _self.$http.post('/system/role/updateRoleResource', data, {
                        emulateJSON: true
                    }).then((response) => {
                        Message({
                            type: 'success',
