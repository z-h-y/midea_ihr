...
<div class="ihr-position-addAnnualPerformance content-wrap">
  <panel :title="panelTitle" class="panel-b mb-suitable" header="panel-header">
      <ui-tabs type="text" background-color="clear" text-color="gray" text-color-active="primary" :active-tab="activeTab" @active-tab-changed="changeTabs">
          <ui-tab header="Basic Information" id = 'basic' :disabled='isAdd'>
            <v-form v-ref:annualForm :model="group" :schema="groupSchema" label-width="150" label-suffix="" :cols="1">
              ...
              <text-field property='resultConfirmedEmpName'  type="selector" :readonly="true" :show.sync="show" editor-width="400"></text-field>
              <div class='field'>
                  ...
                  <div class="field-content" style="margin-left:150px;">
                    ...
                    <div class="field-row position-name-number">
                        <div class="cell">
                            <text-field v-ref:positionnumber label-width="0" :hide-label="true" property="businessUnitProportion" editor-width="100" type = "number">%</text-field>
                        </div>
                        <div class="cell">
          ...
          <ui-tab header="Employees" id = 'employees' :disabled='isAdd'>
            ...
            <ui-modal id="edit-weight" :show.sync="show.weightModal" header="Weight Setting">
              <v-form v-ref:weightform  :model="weightModel" :schema="weightSchema" label-width="150" label-suffix="" :cols="1" form-style="">
          ...
          <ui-tab header="Ratio" id = 'ratio' :disabled='isAdd'>
            <v-form v-ref:annualratio :model="ratioModel" :schema="ratioSchema" label-width="150" label-suffix="" :cols="1">
                <div class="field-row">
                  <div class="cell"><label>Default Ratio</label></div>
                  <div class="cell"><label>Excellent</label></div>
                  <div class="cell"><text-field label-width="0" :hide-label="true" property="excellentRatio" editor-width="100">%</text-field></div>
                </div>
                <div class="field-row">
                  <div class="cell"><label></label></div>
                  <div class="cell"><label>Very Good</label></div>
                  <div class="cell"><text-field label-width="0" :hide-label="true" property="veryGoodRatio" editor-width="100">%</text-field></div>
                </div>
                <div class="field-row">
                  <div class="cell"><label></label></div>
                  <div class="cell"><label>Good</label></div>
                  <div class="cell"><text-field label-width="0" :hide-label="true" property="goodRatio" editor-width="100">%</text-field></div>
                </div>
                <div class="field-row">
                  <div class="cell"><label></label></div>
                  <div class="cell"><label>Needs Improvement</label></div>
                  <div class="cell"><text-field label-width="0" :hide-label="true" property="needsRatio" editor-width="100">%</text-field></div>
                </div>
                <div class="field-row">
                    <div class="cell"><label></label></div>
                    <div class="cell"><label>Disqualified</label></div>
                    <div class="cell"><text-field label-width="0" :hide-label="true" property="disqualifiedRatio" editor-width="100">%</text-field></div>
                </div>
            </v-form>
            <div class="btn-group">
    ...
    data() {
      ...
      let groupSchema = new Schema({
          ...
          return {
              ...
              itemActions: [
                { name: 'edit-item', label: 'Weights setting', icon: '', class: 'operate'},
        ...
        watch: {
          'group.schemeName': {
            handler: function (val, oldVal) {
              ...
              this.$http.get(`/performance/schemeYears/validSchemeName`,{params:{schemeName:val}}).then((response)=>{
        ...
        methods: {
          ...
          basicNext() {
            ...
            let pass = _aPerformance.$schema.isFormValidate(this.$refs.annualform);
            if(!pass) {
              this.isAdditonRight = true;
              return;
            }
          ...
          tabChange(tabId,aid,back) {
              ...
              // let name = !this.schemeId ? 'addAnnualPerformance' : 'editAnnualPerformance';
          ...
          saveWeight() {
            ...
            //验证表单
            let pass = weightModel.$schema.isFormValidate(this.$refs.weightform);
            if (!pass) {
              this.isAdditonRight = true;
              return;
            };
          ...
          submit() {
            // let passed = this.ratioModel.$schema.isFormValidate(this.$refs.annualratio);
            // if (!passed) return;
        ...
        route: {
            data(transition) {
                ...
                if (routeName === 'addAnnualPerformance') {
                  ...
                  _self.panelTitle = 'Edit Annual Scheme';
                  if(step === 'basic') {
                    // initFormData('/performance/yearPfmScores/basicInfo',_self.group,{schemeYearId:this.schemeId});
                    _self.$http.get(`/performance/yearPfmScores/basicInfo`,{params:{schemeYearId:this.schemeId}}).then((response)=>{
                    ...
                    // initFormData('/performance/yearPfmScores/employees',_self.weightModel,{schemeYearId:this.schemeId});
                  } else if (step === 'ratio') {
